...
y_internal = np.var(ctrl_scaled, axis=0)      # internal variability added as noise in kernel
...
def predict_function(i, y, ypred, sd, sd_gp, X, Xtest):
    ...
    kern1.variance = 1e-5
    kern1.lengthscale = [0.89, 0.83, 0.94, 1.11, 0.37, 1., 0.96, 0.93, 0.75]           # Start optimisation from here, found to be good for mean
    kern2 = GPy.kern.Linear(p,ARD=True)
    kern2.variances = [0.63, 0.03, 0.01, 0.001, 0.007, 0.006, 0.003, 0.037, 0.015]     # Start optimisation from here
    covar_fixed = y_internal[i] * np.eye(X.shape[0])
    kern_fixed =  GPy.kern.Fixed(p, covar_fixed)                                       # Fixed white noise for invernal variation
    kern = kern1 + kern2  + kern_fixed
    m = GPy.models.GPRegression(X, y_i, kern)
    ###m.likelihood.variance.fix(y_internal[i])
    m.optimize('bfgs', max_iters=1000)

    ### PREDICT ####
    covar_fixed_pred = y_internal[i] * np.eye(Xtest.shape[0])
    kern_fixed_pred =  GPy.kern.Fixed(p, covar_fixed_pred)
    kern_pred = kern1.copy() + kern2.copy() + kern_fixed_pred
    ypred_i, sd_full_i = m.predict(Xtest, kern = kern_pred)
    _, sd_GP_i = m.predict_noiseless(Xtest,  kern = kern_pred)
    Ntest = len(ypred_i)
...
if __name__ == '__main__':
    ...
    save_file = '../../emulator_outputs/indepdims_GP_with_internalnoise_fixedkernel.file'
