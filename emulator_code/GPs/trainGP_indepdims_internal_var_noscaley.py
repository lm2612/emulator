...
def predict_function(i, y, ypred, sd, sd_gp, X, Xtest):
    ...
    kern1.lengthscale = [0.49, 0.29, 0.53, 0.42, 0.04, 0.65, 0.46, 0.44, 0.15]
    kern2 = GPy.kern.Linear(p,ARD=True)
    kern2.variances = [0.62, 0.02, 0.01, 0.01, 0.01, 0.005, 0.002, 0.03, 0.015]  
    kern = kern1 + kern2
    m = GPy.models.GPRegression(X, y_i, kern)
    m.likelihood.variance.fix(y_internal[i])
    m.optimize('bfgs', max_iters=500)
...
if __name__ == '__main__':
    ...
    save_file = '../../emulator_outputs/indepdims_GP_with_internalnoise_noscaley.file'
