...
N = 100
Xtest_all = np.zeros((9, N, 9))
for j in range(9):
  Xtest = np.zeros((N, 9))
  for i in range(9):
    min_i, base_i, max_i = min_vals[i], base_vals[i], max_vals[i]
    if i==j:
        Xtest[:, i] = np.linspace(min_i, max_i, N)
    else:
        Xtest[:, i] = np.random.rand(N)*(max_i-min_i) + min_i

  Xtest_scaled = scalerX.transform(Xtest)
  Xtest_all[j, :, : ] = Xtest_scaled
Xtest = Xtest_all.reshape((N*9, 9))
...
if __name__ == '__main__':
    ...
    save_file = '../../emulator_outputs/totaleffects.file'
