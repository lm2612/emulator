...
filekeys, _, _, _, temp = open_file('../../emulator_files/AllOATRuns.nc')
sorted_inds = np.argsort(filekeys)
print(filekeys)
sorted_keys = filekeys[sorted_inds]
...
ctrl_temps = temps[0]
em_temps = temps[1:]
YOAT = em_temps - ctrl_temps
print(YOAT.shape)
XOAT = (np.genfromtxt('../../design/OAT_matrix.csv', delimiter=',',
                 skip_header=1, usecols=range(1,10)))

print(np.sum(np.isnan(YOAT)))
print(YOAT[:,0,0])
print(YOAT.shape)
print(XOAT.shape)
inds = list(range(0,6))+list(range(7,9))+list(range(10,12)) + list(range(13,21))
XOAT = XOAT[inds,:]
YOAT = YOAT[inds,:]
N = X.shape[0]
N_oat = XOAT.shape[0]
Xfull = np.zeros((N+N_oat, X.shape[1]))
Yfull = np.zeros((N+N_oat, Y.shape[1], Y.shape[2]))
Xfull[0:N] = X
Xfull[N:] = XOAT
Yfull[0:N] = Y
Yfull[N:] = YOAT
...
if __name__ == '__main__':
    ...
    save_file = '../../emulator_outputs/maineffects_oat.file'
    #save_objs = {'ypred':ypred, 'Xtest':Xtest, 'sd':sd, 'sd_GP':sd_gp}
    save_objs = {'y_avg':y_avg, 'y_sd': y_sd}
