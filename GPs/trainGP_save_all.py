...
def predict_function(i, y, ypred, sd, sd_gp, X, Xtest, ms):
    y_i = y[:, i]
    N = len(y_i)
    y_i = y_i.reshape((N, 1))
...
if __name__ == '__main__':
    ...
    jobs = []
    #ms = mp.sharedctypes.RawArray('d', k)
    ms  = mp.Queue()
    for i in range(k):
        proc= mp.Process(target=predict_function, args=(i, y, ypred_arr, sd_arr, sd_gp_arr, X, Xtest, ms))
        jobs.append(proc)
        proc.start()
    for j in jobs:
        j.join()
    ...
    print("DONE")
    save_file = '../../emulator_outputs/indepdims_GP_with_internalnoise_model'
    save_objs = {'models':ms}
    pickle.dump(save_objs, file=open(save_file, "wb"))
    print("Saved objs in ", save_file)
    print("DONE")
