...
for j in range(9):
  ...
  for i in range(9):
    ...
    if i==j:
        ...
        # Random for all N function replications, uniform from min to base and then uniform from base to max 
        min_to_base = np.random.randn(int(N/2)) * (base_i - min_i) + min_i
        base_to_max = np.random.randn(int(N/2)) * (max_i - base_i) + base_i
        for g in range(grid_size):
            Xtest[:int(N/2), g, i] = min_to_base
            Xtest[int(N/2):, g, i] = base_to_max
...
if __name__ == '__main__':
    ...
    save_file = '../../emulator_outputs/maineffects_avg_unif.file'
