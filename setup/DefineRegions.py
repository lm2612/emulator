...
def DefineRegion(region,lons,lats,regiondict):
    ...
    (lonmin,lonmax,latmin,latmax)=regiondict[region]
    # Initialise grid
...
def AreaRegion(region,lons,lats,regiondict):
        nlon , nlat = len(lons), len(lats)
        # Get min and max lons and lats from dict in RegionLatitudes
        lonmin,lonmax,latmin,latmax=regiondict[region]
        # area
        dcoslat = np.cos((latmax)*np.pi/180.) - np.cos((latmin)*np.pi/180.)
        dlon = (lonmax - lonmin)*np.pi/180.
        area = np.abs((dcoslat*dlon)/(4*np.pi))
        return area
